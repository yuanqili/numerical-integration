...
st.sidebar.header('Simpson\'s Rule')
...
for a, b in zip(divide_xs[:-1], divide_xs[1:]):
    ...
    fm = stats.norm.pdf(m, mu, sigma)

    x = sym.symbols('x')
    px = fa * (x - m) * (x - b) / ((a - m) * (a - b)) \
         + fm * (x - a) * (x - b) / ((m - a) * (m - b)) \
         + fb * (x - a) * (x - m) / ((b - a) * (b - m))
    px = sym.lambdify(x, px)
    px_xs = np.linspace(a, b, 100)
    px_ys = px(px_xs)
    axes.plot(px_xs, px_ys, color=color, lw=0.5)
    axes.fill_between(px_xs, px_ys, color=color, alpha=0.2)

    area = ((b - a) / 6) * (fa + 4 * fm + fb)
    areas.append({'a': a, 'b': b, 'width': width, 'area': area})
    if show_area:
        axes.text(m, 0.02, f'{area:.2f}', ha='center', va='bottom', fontsize=10,
